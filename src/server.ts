...
import { validateToken, generateError, decodeToken, decryptData, encryptData } from "./controllers/common"
            ...
            switch (request.method.toLowerCase()) {
                ...
                case 'get':
                    if(typeof request.query!='object')
                        request.query = JSON.parse(JSON.stringify(request.query));
...
export const startServer = async (server: Server) => {
    try {
        ...
        global.io = socketInstance(server.listener, { cors: { origin: '*' } });
