...
const listResponse: Joi.ObjectSchema = Joi.object().keys({
    ...
    responseData: Joi.object().keys({
        ...
        page: Joi.number().integer().example(1).description("page no for which data is requested"),
        totalRecords: Joi.number().example(1).description("total number of reccords"),
        totalPages: Joi.number().integer().example(1).description("Total number of pages response set will generate")
