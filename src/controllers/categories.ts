...
import { CategoryInterface, CategoryContentInterface, CategoryAttributeContentInterface, CategoryAttributeOptionContentInterface } from "../config/interfaces/category";
...
const attributes: AttributeElement[] = [
    ...
    [sequelize.literal('(case when `content`.name is not null then `content`.name else `defaultContent`.name END)'), 'name'],
    [sequelize.literal(`(SELECT count(id) from categories where parent_id = Category.id and deleted_at is null and is_revision = 0)`), "childCategoryCount"] 
];

const categoryImageAttributes: AttributeElement[] = [
    "id",
    [sequelize.fn('CONCAT', process.env.BASE_URL, "/attachment/", sequelize.literal('`categoryImage`.`unique_name`')), 'filePath']
]
...
const categoryParentImageAttributes: AttributeElement[] = [
    "id",
    [sequelize.fn('CONCAT', process.env.BASE_URL, "/attachment/", sequelize.literal('`parent->categoryImage`.`unique_name`')), 'filePath']
]
...
const categoryAttributeAttributes: AttributeElement[] = [
    'id', 'code', 'categoryId', 'type', 'isVariant', 'status', 'createdAt', 'updatedAt',
    [sequelize.literal('(case when `content`.name is not null then `content`.name else `defaultContent`.name END)'), 'name'],
];

const categoryAttributeOptionAttributes: AttributeElement[] = [
    'id', 'code', 'categoryAttributeId',
    [sequelize.literal('(case when `CategoryAttributeOptions->content`.name is not null then `CategoryAttributeOptions->content`.name else `CategoryAttributeOptions->defaultContent`.name END)'), 'name'],
];
...
export const deleteCategory = async (request: Hapi.RequestQuery, h: Hapi.ResponseToolkit) => {
    ...
    try {
        ...
        if (category) {
            const childCategory = await Models.Category.findOne({ where: { parentId: id, isRevision: false } });
            if(childCategory) {
                await transaction.rollback();
...
export const list = async (request: Hapi.RequestQuery, h: Hapi.ResponseToolkit) => {
    try {
        ...
        let where: WhereOptions = { isRevision: false, parentId: parentId, categorytypeId: categoryType.id }
        if (type == 'directory') {
...
const getCategoryTree = async (categotyTypeId:number,parentId:number|null,language:string) => {
    try {
      ...
      if (categories.length > 0) {
        ...
        for (let category of categories) {
          ...
          if (category.name && category.id) {
            ...
            let childTree = await getCategoryTree(categotyTypeId, category.id, language);
...
// get a category type by id
export const getTree = async (request: Hapi.RequestQuery, h: Hapi.ResponseToolkit) => {
    try {
        let { categoryTypeCode } = request.params;
        let accountId = request.auth.credentials.userData.accountId;
        ...
        const responseData = await getCategoryTree(categorytypeDetails.id!, null, request.headers.language);
        if (responseData) {
