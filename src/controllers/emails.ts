...
import { EmailTemplate,EmailTemplateContent } from "../config/interfaces/email";
...
export const create = async (request: Hapi.RequestQuery, h: Hapi.ResponseToolkit) => {
    ...
    try {
        ...
        let userId = request.auth.credentials.userData.userId;
        ...
        if (emailTemplate) {
            ...
            return h.response({ message: hapi.request.i18n.__("EMAIL_TEMPLATE_CREATED_SUCCESSFULLY"), responseData: returnOBj }).code(200);
        } else {
            await transaction?.rollback();
        ...
        if (emailTemplate) {    
            ...
            if (revision) {
                ...
                if (requestedLanguageId) {
                    ...
                    return h.response({ message:  hapi.request.i18n.__("EMAIL_TEMPLATE_HAS_BEEN_UPDATED_SUCCESSFULLY"), responseData: responseObject }).code(200)
                } else {
                    await transaction?.rollback();
...
export const deleteTemplate = async (request: Hapi.RequestQuery, h: Hapi.ResponseToolkit) => {
    ...
    try {
        let userId = request.auth.credentials.userData.id;
        let { id } = request.params;
        let emailTemplate = await Models.EmailTemplate.findOne({ where: { id: id } });
        if (emailTemplate) {
            ...
            if (revision) {
                ...
                await transaction?.commit();
                return h.response({ responseData: (revisonObject), message: hapi.request.i18n.__("EMAIL_TEMPLATE_DELETED_SUCCESSFULLY") }).code(200);
            } else {
                await transaction?.rollback();
                return Common.generateError(request, 400, 'ERROR_WHILE_CREATING_REVISION', {});
            }
...
export const get = async (request: Hapi.Request, h: Hapi.ResponseToolkit) => {
    try {
        let { id } = request.params
        let emailTemplate = await fetch(id, request.headers.language);
        if (emailTemplate) {
            return h.response({ message: hapi.request.i18n.__("REQUEST_PROCESSED_SUCCESSFULLY"), responseData: JSON.parse(JSON.stringify(emailTemplate)) }).code(200)
        } else {
            return Common.generateError(request, 400, 'EMAIL_TEMPLATE_DOES_NOT_EXISTS', {});
        }
    } catch (err) {
