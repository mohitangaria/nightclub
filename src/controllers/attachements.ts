        ...
        if (request.payload && request.payload['file']) {
            if (process.env.USE_FILE_SYSTEM) {
                ...
                if ((fileDetails && fileDetails.hasOwnProperty('uniqueName')) || (Array.isArray(fileDetails) && fileDetails && fileDetails.length)) {
                    ...
                    respData['filePath'] = process.env.PROTOCOL+'://'+process.env.HOST_SERVER +'/attachment/'+ respData.uniqueName
                    return h.response({
                        message: request.i18n.__("FILE_UPLOADED_SUCCESSFULLY"),
                ...
                if(true){
                    ...
                    responseData['filePath'] = process.env.PROTOCOL+'://'+process.env.HOST_SERVER  +'/attachment/'+ responseData.fileName;
                    return h.response({
